        ...
        <NavbarBrand as="li" className="gap-3 max-w-fit">
          <NextLink className="flex justify-start items-center gap-1" href="/">
            <Image
              ...
              width={40}
            />
            <p className="font-bold text-inherit">MotorShield</p>
          </NextLink>
        </NavbarBrand>
        <ul className="hidden lg:flex gap-10 justify-start ml-20">
        ...
        <div className="mx-4 mt-2 flex flex-col gap-2">
          {siteConfig.navMenuItems.map((item, index) => (
            <NavbarMenuItem key={`${item}-${index}`}>
              <Link
                className="w-full" // Added full width for click area
                color="foreground"
                href={item.href}
                size="lg"
              >
